...
ax4.plot(f[7:], (S_10Mar1500UT_T1*get_cg(f, h))[7:], color='k', linewidth=1.5, label="Boundary input")
ax4.scatter(f_10Mar1500UT_T1_Obs, S_10Mar1500UT_T1_Obs*get_cg(f_10Mar1500UT_T1_Obs, h), color='#DB494E', s=30, label="Observation")
ax4.set_ylim(2e-3,50)
...
ax4.legend([handles[i] for i in order],
           [labels[i] for i in order], loc="upper right",
           frameon=False)
ax4.text(0.05, 0.9, 'T1', transform=ax4.transAxes, ha='left', va='top', fontweight='bold')

ax5 = fig.add_subplot(gs[3, 0:2])
ax5.scatter(f1, S_10Mar1500UT_T2_Obs, color='#DB494E', s=30, label="Observation")
ax5.text(0.05, 0.9, 'T2', transform=ax5.transAxes, ha='left', va='top', fontweight='bold')
ax5.plot(f_T2[11:], (S_10Mar1500UT_T2*get_cg(f_T2,h))[11:], color='k', linewidth=1.5, label="Prediction")
ax5.set_ylim(2e-3,50)
handles, labels = ax5.get_legend_handles_labels()

# # Define your desired order by index
order = [0,1]  # Change this to the order you want

# # Apply reordered legend
ax5.legend([handles[i] for i in order],
           [labels[i] for i in order],
           frameon=False)

ax6 = fig.add_subplot(gs[2, 2:4])
ax6.plot(f[7:], (S_10Mar2100UT_T1*get_cg(f, h))[7:], color='k', linewidth=1.5, label="Boundary input")
ax6.scatter(f_10Mar2100UT_T1_Obs, S_10Mar2100UT_T1_Obs*get_cg(f_10Mar2100UT_T1_Obs, h), color='#DB494E', s=30, label="Observation")
ax6.set_ylim(2e-3,50)
...
ax7.scatter(f1, S_10Mar2100UT_T2_Obs, color='#DB494E', s=30, label="Observation")
ax7.plot(f_T2[11:], (S_10Mar2100UT_T2*get_cg(f_T2,h))[11:], color='k', linewidth=1.5, label="Prediction")
ax7.text(0.05, 0.9, 'T2', transform=ax7.transAxes, ha='left', va='top', fontweight='bold')
ax7.set_ylim(2e-3,50)
...
# Apply reordered legend
ax7.legend([handles[i] for i in order],
           [labels[i] for i in order],
           frameon=False)

ax8 = fig.add_subplot(gs[2, 4:6])
ax8.plot(f[7:], (S_11Mar1100UT_T1*get_cg(f, h))[7:], color='k', linewidth=1.5, label="Boundary input")
ax8.scatter(f_11Mar1100UT_T1_Obs, S_11Mar1100UT_T1_Obs*get_cg(f_11Mar1100UT_T1_Obs, h), color='#DB494E', s=30, label="Observation")
ax8.set_ylim(1e-3,50)
...
ax9.scatter(f1, S_11Mar1100UT_T2_Obs, color='#DB494E', s=30, label="Observation")
ax9.plot(f_T2[11:], (S_11Mar1100UT_T2*get_cg(f_T2,h))[11:], color='k', linewidth=1.5, label="Prediction")
ax9.text(0.05, 0.9, 'T2', transform=ax9.transAxes, ha='left', va='top', fontweight='bold')
ax9.set_ylim(1e-3,50)
...
# Apply reordered legend
ax9.legend([handles[i] for i in order],
           [labels[i] for i in order],
           frameon=False)
...
for ax, label in zip(axes, labels):
    ...
    fig.text(x_fig - 0.035, y_fig-0.01, f"{label}", fontsize=18, fontweight='bold')
...
for ax, label in zip(axes, labels):
    ...
    # Apply a small offset
    fig.text(x_fig - 0.035, y_fig, f"{label}", fontsize=18, fontweight='bold')
