...
# Hsu13
t1_Hsu13NM, eta1_Hsu13NM = Exp['t_x6.5m_Hsu13NM'].dropna(), Exp['eta_x6.5m_Hsu13NM'].dropna()
t2_Hsu13NM, eta2_Hsu13NM = Exp['t_x9.76m_Hsu13NM'].dropna(), Exp['eta_x9.76m_Hsu13NM'].dropna()
t3_Hsu13NM, eta3_Hsu13NM = Exp['t_x11.93m_Hsu13NM'].dropna(), Exp['eta_x11.93m_Hsu13NM'].dropna()
...
Hx_Case2CNM = Exp['Hx_CASE2C_Hsu13NM'].dropna()
Hx_Case4C = Exp['Hx_CASE4C_Hsu13'].dropna()
Hx_Case4CNM = Exp['Hx_CASE4C_Hsu13NM'].dropna()
...
U1_Case2B_Hsu13NM = Exp['U_crest_CASE2B_Hsu13NM'].dropna()
z_Case2B_Hsu13NM = Exp['U_crest_CASE2B_Hsu13NM_z'].dropna()
z = Exp['z_Hsu13'].dropna()
U2_Case2B = Exp['U_reversal_CASE2B_Hsu13'].dropna()
U2Err_Case2B = Exp['Error_U_reversal_CASE2B_Hsu13'].dropna()
U2_Case2B_Hsu13NM = Exp['U_reversal_CASE2B_Hsu13NM'].dropna()
z2_Case2B_Hsu13NM = Exp['U_reversal_CASE2B_Hsu13NM_z'].dropna()
theta_Case2B = Exp['theta_CASE2B_Hsu13'].dropna()
theta_Case2B_Hsu13NM = Exp['theta_CASE2B_Hsu13NM'].dropna()
z3_Case2B_Hsu13NM = Exp['theta_CASE2B_Hsu13NM_z'].dropna()
...
line_kwargs = dict(color='black', linestyle='--', linewidth=1.5, zorder=2, label='This study')

ax1 = fig.add_subplot(gs[0, 0:3])
# Plot each subplot
ax1.scatter(t1, eta1, **scatter_kwargs)
ax1.plot(t1_Hsu13NM, eta1_Hsu13NM, color = 'gray', linestyle=':', linewidth=1.5, label='Hsu13 NM')
ax1.plot(t_modeled, eta1_modeled, **line_kwargs)
ax1.set_ylabel(r"$\eta$ (m)", fontsize = label_font_size)

ax2 = fig.add_subplot(gs[1, 0:3])
ax2.scatter(t2, eta2, **scatter_kwargs)
ax2.plot(t2_Hsu13NM, eta2_Hsu13NM, color = 'gray', linestyle=':', linewidth=1.5, label='Hsu13 NM')
ax2.plot(t_modeled, eta2_modeled, **line_kwargs)
ax2.set_ylabel(r"$\eta$ (m)", fontsize = label_font_size)

ax3 = fig.add_subplot(gs[2, 0:3])
ax3.scatter(t3, eta3, **scatter_kwargs)
ax3.plot(t3_Hsu13NM, eta3_Hsu13NM, color = 'gray', linestyle=':', linewidth=1.5, label='Hsu13 NM')
ax3.plot(t_modeled, eta3_modeled, **line_kwargs)
ax3.set_ylabel(r"$\eta$ (m)", fontsize = label_font_size)
ax3.set_xlabel(r"$t$ (s)", fontsize = label_font_size)

# Add legend only to the last subplot
ax1.legend(fontsize=legend_font_size, loc='upper right',bbox_to_anchor=(1, 1.7), ncol=3)
...
ax4.errorbar(x, Hx_Case2C, yerr=error1_Hx_Case2C, fmt='o', markersize=6, color='#1f77b4', elinewidth=1, capsize=2, label='Case 2C - Exp')
ax4.errorbar(x, Hx_Case4C, yerr=error2_Hx_Case4C, fmt='s', markersize=6, color='#DB494E', elinewidth=1, capsize=2, label='Case 4C - Exp')

ax4.scatter(x, Hx_Case2CNM, 30, marker='s', facecolors='none', edgecolors='k', label='Hsu13 NM')
ax4.scatter(x, Hx_Case4CNM, 30, marker='s', facecolors='none', edgecolors='k', label='Hsu13 NM')

# Plot modeled data
ax4.plot(x_modeled, Hx_Case2C_modeled, linestyle='--', color='k', linewidth=1.5, label='This study')
...
order = [3, 4, 0, 2]  # Change this to the order you want
...
# Hsu13
ax5.plot(measured_hsu, predicted_hsu, marker='s', markersize=6, linestyle='None', fillstyle='none', markeredgecolor='k', label=f'Hsu13 NM, $R^2$ = {r2_Hsu13:.2f}')
...
ax6.errorbar(U1_Case2B, z, xerr=U1Err_Case2B,
            ...
            label='Exp')
ax6.plot(U1_Case2B_Hsu13NM, z_Case2B_Hsu13NM, ':', color = 'grey', label='Hsu13 NM')
...
ax7.plot(U2_Case2B_Hsu13NM, z2_Case2B_Hsu13NM, ':', color = 'grey', label='Hsu13 NM')
...
ax8.plot(theta_Case2B_Hsu13NM, z3_Case2B_Hsu13NM, ':', color = 'grey', label='Hsu13 NM')
...
# Define your desired order by index
order = [2,0,1]  # Change this to the order you want
